...
namespace DAL.classes
{
    public class CategoryDAL : ICategoryDAL
    {
        ...
        public bool Update(int id, Category c)
        {
            try
            {
                // Category old = db.Categories.FirstOrDefault(x => x.CategorId == id);
                db.Categories.FirstOrDefault(x => x.CategorId == id).CategoryName = c.CategoryName;
                //Category category = db.Categories.FirstOrDefault(x => x.CategorId == id);              
                //db.Entry(category).CurrentValues.SetValues(c);  
                db.SaveChanges();
                return true;
            }
            catch 
