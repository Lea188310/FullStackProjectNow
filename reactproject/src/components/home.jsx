...
style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '10px' }}>
    {list.map((x,i)=>(
        ...
        style={{width : '300px'}}
        >
            {/* <img className="card-img-top" src={`https://localhost:7287/${x.pecture}`}></img> */}
 ...
 <Outlet></Outlet>
       ...
       <div className="card text-center mb-3"
                     style={{ width: "18rem" ,borderColor:'black',borderWidth:"2px" ,display:"grid",alignContent:"space-beetween"}}> 
                       <img className="card-img-top" src={`https://localhost:7287/${x.pecture}`}></img>
                        <div className="card-body">
                            <h5 className="card-title">{x.productName}</h5>
                            <p className="card-text">{x.price}₪</p>
                            <button onClick={()=>addnewItem(x)} className="btn btn-outline-dark">הוסף לסל</button>
                            <Link className="btn btn-outline-dark" to={`showGameDetailes/${x.gameId}`}>פרטים נוספים</Link>
                         </div>
                    </div> </>  
                    ))}
                <Outlet></Outlet>
